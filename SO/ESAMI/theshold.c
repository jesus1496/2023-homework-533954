...
void *func(void* args){
    ...
    while((rb = read(fd,buf,sizeof(int)*CHUNK_SIZE)) > 0)
        ...
        for(i=0; i<rb/sizeof(int); i++)
            ...
            //add 1 to total if buf[i] is higher then thresholds
            //this is quite baf performance, do you know why? howcan this improved?
...
/*start solution code*/
      ...
      //get a file descriptor and store on a variable easily accessibily for child threads
      fd = open(filename, O_RDONLY);
      //put threshold ina global variable
      th = threshold;
