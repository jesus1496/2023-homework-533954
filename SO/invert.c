...
int main(int argc, char *argv[]) {
  if(argc != 4){  //se il contatore di argomento è diverso da 4
    printf("Command usage: <input file> <output file> <num_threads>");
    exit(1);  //indica un'interruzione senza successo
  }
  ...
  int input_fd        = open(in_file_path, O_RDONLY); //O_RDONLY: apertura di un file di solo lettura
  int output_fd       = open(out_file_path, O_WRONLY | O_CREAT | O_TRUNC, 0660); //0660: utente/proprietario possono leggere e scrivere ma non eseguire
  size_t read_bytes   = 0;
